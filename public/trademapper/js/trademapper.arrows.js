...
define(["d3v5", "spiralTree", "trademapper.route", "trademapper.portlookup", "trademapper.legend", "util", "config"], function(d3v5, flowmap, tmroute, portlookup, Legend, util, config){
	"use strict";
	return {
	mapsvg: null,
	zoomg: null,
	...
	init: function(svgElement, zoomg, svgDefs, tooltipSelector, colours, minWidth, maxWidth, pointTypeSize, countryCodeToInfo) {
		...
		this.pathTooltip = d3v5.select(tooltipSelector);
	...
	clearArrows: function() {
        d3v5.selectAll('.route-arrow').remove();
	},
	...
	drawRouteCollectionPlainArrows: function(collection, pointRoles, maxQuantity, csvData) {
		this.clearArrows();
		this.clearPoints();
		if (maxQuantity) {
		...
		this.drawPointRoles(pointRoles, csvData);
	},
	...
	hoverPointTooltip: function(pointContainer, pointName, csvData) {
        ...
        //             '<span class="tooltip-country">' + theExport['Country Code'] + '</span>' +
        //             '</div>';
        ...
        //             money_final + unitS +
        //             '</span>' +
        //             '<span class="tooltip-total">Outbound <br>Spend:</span>' +
        //             '<span class="tooltip-units">' +
        //             unit +
        //             '</span>' +
        //             '<span class="tooltip-country">' + theAssets['Country Code']  + '</span>' +
        //             ranking1 +
        //             ranking2 +
        //             '</div>';
        ...
        //             unit +
        //             '</span>' +
        //             '<span class="tooltip-country">' + theTravel['Country Code'] + '</span>' +
        //             ranking1 +
        //             ranking2 +
        //             '</div>';
        //     } else {
        ...
        pointContainer
            .on("mouseover", function () {
                ...
                //     '<div class="">' + getImportData(pointName) + '</div>' +
                //     '<br/>')
                //     .style("width", "17em")
                //     .style("height", "9.6em")
	...
	drawPointRole: function(point, pointType, pointName, csvData) {
		if (point === undefined) {
			console.log("point undefined for " + pointName);
			return;
		}
		this.hoverPointTooltip(this.drawPoint(point[0], point[1], pointType, "", this.arrowg), pointName ,csvData);
	},

	drawPointRoles: function(pointRoles, csvData) {
		var pointInfo,
		...
		for (var i = 0; i < pointKeys.length; i++) {
			...
			for (var j = 0; j < tmroute.locationRoles.length; j++) {
				...
				if (pointInfo.roles.roles[role]) {
					this.drawPointRole(pointInfo.point, role, pointKeys[i], csvData);
				}
			}
		}
	},
	...
	clearPoints: function() {
        d3v5.selectAll(".tradenode").remove();
	},
	...
	showPathTooltip: function(tooltiptext, tooltipWidth, tooltipHeight) {
        ...
        d3v5.select("span.tooltip-close")
	...
	plainMouseOverPath: function(route) {
        ...
        d3v5.selectAll("." + this.highlightedPath).style("stroke", '#FF0000');
		...
		tooltiptext += '<span class="tooltip-total">Exporting to:</span><br />';
			...
			if (pointsWithRole.length) {
				...
				for (var j = 0; j < pointsWithRole.length; j++) {
					...
					tooltiptext += ' <span class="location-role-country ' +
						code + '"' + titleAttr + '>' + this.countryCodeToInfo[code].name + '</span>';
				}
				tooltiptext += '</p>';
			}
		}
	...
	plainMouseOutPath: function() {
		// remove path stroke
        d3v5.selectAll("." + this.highlightedPath).style('stroke', null);
		this.setPathOpacity(this.highlightedPath, this.arrowColours.opacity);
	...
	flowmapMouseOverPath: function(ctIndex) {
        ...
        d3v5.selectAll(".traderoute-highlight").classed("traderoute-highlight", false);
		var allpath = d3v5.selectAll(pathSelector);
