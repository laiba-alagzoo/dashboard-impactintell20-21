define(["d3v5", "jquery", "config"], function (d3v5, $, config) {
	// constructor
	//
	...
	return function (mapsvg) {
		...
		var makeRouteElements = function (fontSizePx) {
			...
			for (i = 3; i >= 0; i--) {
				...
				var routeLine = d3v5.create("svg:rect");
				...
				var routeText = d3v5.create("svg:text");
		...
		var makeTradenodeElements = function (fontSizePx) {
			...
			for (var i = 0; i < possibleRoles.length; i++) {
				if (state.locationRoles.indexOf(possibleRoles[i]) !== -1) {
					...
					var graphic = d3v5.create("svg:circle");
					...
					var label = d3v5.create("svg:text");
		...
		var makeLayerElements = function (fontSizePx) {
			...
			for (var i = 0; i < state.layers.length; i++) {
				var graphic = d3v5.create("svg:rect");
		...
		legendObj.draw = function () {
            ...
            d3v5.select("#legendcontainer").remove();
