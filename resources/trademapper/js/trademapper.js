...
define([
    ...
    function ($, d3v5, analytics, arrows, csv, filterform, mapper, route, yearslider,
        ...
        return {
            ...
            init: function (mapId, fileFormElementId, layerFormElementId, filterFormElementId,
                ...
                this.mapRootElement = d3v5.select(mapId);
                this.fileFormElement = d3v5.select(fileFormElementId);
                this.filterFormElement = d3v5.select(filterFormElementId);
                this.svgExportButtonElement = d3v5.select(svgExportButtonElementId);
                this.pngExportButtonElement = d3v5.select(pngExportButtonElementId);
                this.videoExportButtonElement = d3v5.select(videoExportButtonElementId);
                this.changeOverTimeElement = d3v5.select(changeOverTimeElementId);
                ...
                csv.init(
                    ...
                    function (csvData, filterSpec, filters) {
                        moduleThis.filterLoadedCallback(csvData, filterSpec, filters);
                    },
                    function () {
                        moduleThis.csvLoadErrorCallback();
                    },
                ...
                route.setCountryGetPointFunc(function (countryCode) {
                    return mapper.countryCentrePoint(countryCode);
                });
                route.setPortGetPointFunc(function (portCode) {
                    return mapper.portCentrePoint(portCode);
                });
                route.setLatLongToPointFunc(function (latLong) {
                    return mapper.latLongToPoint(latLong);
                });
                filterform.formChangedCallback = function (columnName) {
                    return moduleThis.filterformChangedCallback(columnName);
                };
                yearslider.showTradeForYear = function (year) {
                    return moduleThis.showTradeForYear(year);
                };
                // slightly misnamed as we go back to the filter values for the years
                yearslider.showTradeForAllYears = function () {
                    return moduleThis.filterformChangedCallback();
                };
                yearslider.enableDisableCallback = function (enable) {
                    return moduleThis.yearSliderEnableDisableCallback(enable);
                };
            ...
            createCsvOnlyForm: function () {
                ...
                $('form#tm-file-select').submit(function () {
                    return false;
                });
            ...
            addChangeFilterSpecToDataTab: function () {
                var dataFilterSpecChange = d3v5.select(".change-filter-spec");
            ...
            createFilterForm: function (filters) {
                ...
                document.querySelector('[role=filters]').style.display = "block";
                document.querySelector('[role=export]').style.display = "block";
            },
     ...
     */
            showFilteredCsv: function (filterValues, maxQuantity, csvData) {
                this.showNowWorking();
                arrows.clearTooltip();
                ...
                this.drawArrows(routes, pointRoles, maxQuantity, csvData);

                // colour in the countries that are trading and/or countries containing a port
                // which is trading
                mapper.setTradingCountries(routes.getCountryCodes(), csvData);
                mapper.setTradingCountriesExports(routes.getCountryCodes(), csvData);
                this.stopNowWorking();
            },

            drawArrows: function (routes, pointRoles, maxQuantity, csvData) {
                arrows.currentUnit = this.currentUnit;
                // now draw the routes
                if (this.config.arrowType === "plain-arrows") {
                    arrows.drawRouteCollectionPlainArrows(routes, pointRoles, maxQuantity, csvData);
            ...
            csvLoadedCallback: function (csvFileName, csvData, csvFirstTenRows, filterSpec) {
                ...
                this.updateMapperZoom();
                this.updateMaxSingleYearQuantity();
                this.showFilteredCsv(filterform.filterValues, undefined, csvData);
            ...
            reportCsvLoadErrors: function () {
                var errorDetails,
                    ...
                    errorFieldset = d3v5.select(".csv-load-errors");
